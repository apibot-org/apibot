...
The `home-routes` handler in the `apibot.routes.home` namespace
...
The `home-page` function will in turn call the `apibot.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/apibot/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `apibot.handler` namespace:
...
Request middleware functions are located under the `apibot.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `apibot.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `apibot.core` in some other namespace.
